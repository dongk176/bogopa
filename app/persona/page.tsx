...
function CustomDropdown({ label, options, value, onChange }: { label: string, options: string[], value: string, onChange: (val: string) => void }) {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <label className="text-sm font-bold text-[#afb3ac] uppercase block mb-2">{label}</label>
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="flex w-full items-center justify-between rounded-2xl bg-[#f4f4ef] px-5 py-4 text-base font-bold text-[#2f342e] transition-all hover:bg-[#ecece4] active:scale-[0.98]"
            >
                <span>{value}</span>
                <svg className={`h-4 w-4 text-[#afb3ac] transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                </svg>
            </button>

            {isOpen && (
                <div className="absolute left-0 top-full z-[120] mt-2 w-full overflow-hidden rounded-2xl border border-[#afb3ac]/10 bg-white shadow-xl animate-fade-in">
                    {options.map((opt) => (
                        <button
                            key={opt}
                            onClick={() => {
                            ...
                            className={`flex w-full items-center px-5 py-4 text-left text-base font-bold transition-colors ${value === opt ? "bg-[#4a626d] text-white" : "text-[#2f342e] hover:bg-[#f4f4ef]"
                                }`}
                        >
                            {opt}
                        </button>
...
export default function PersonaPage() {
    ...
    const handleOpenDetail = (persona: Persona) => {
        ...
        setEditForm({
            name: persona.name,
            relation: rt?.relation || al?.personaInput.relation || "",
    ...
    const renderListEditor = (label: string, list: string[], setList: (next: string[]) => void, placeholder = "내용을 입력하세요") => (
            ...
            <main className="mx-auto max-w-5xl px-6 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-12 md:pt-20 md:pb-20 lg:pl-64">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center rounded-[2.5rem] bg-white p-20 text-center shadow-sm border border-[#afb3ac]/10">
                        ...
                        <p className="mt-2 text-[#655d5a]">첫 번째 페르소나를 만들어 대화를 시작해보세요.</p>
                        <button
                            ...
                            <div className="flex items-center gap-3">
                                {!isEditing ? (
                                    ...
                                    <button
                                        ...
                                        className="rounded-xl px-4 py-2 text-sm font-bold text-[#655d5a] hover:bg-[#f4f4ef] transition-colors"
                                    >
                                        취소
                                    </button>
                                )}
                        ...
                        <div className="flex-1 overflow-y-auto p-8 lg:p-12 scrollbar-hide">
                            <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
                                ...
                                <div className="space-y-12">
                                    ...
                                    <section>
                                        <h3 className="mb-4 text-sm font-extrabold uppercase tracking-[0.2em] text-[#4a626d]">대화 스타일 (선택형)</h3>
                                    ...
                                    <section>
                                        ...
                                        <div className="space-y-6 rounded-[2rem] bg-white p-6 md:p-8 shadow-sm border border-[#afb3ac]/10">
                                            {isEditing ? (
                                                <>
                                                    {renderListEditor("웃음 습관 (ㅋㅋ, ㅎㅎ 등)", editForm.laughterPatterns, (next) => setEditForm({ ...editForm, laughterPatterns: next }))}
                                                    {renderListEditor("슬픔 습관 (ㅠㅠ, ... 등)", editForm.sadnessPatterns, (next) => setEditForm({ ...editForm, sadnessPatterns: next }))}
                                                </>
                                            ) : (
                                                <div className="grid grid-cols-2 gap-6">
                                                    <div>
                                ...
                                <div className="space-y-12">
                                        ...
                                        <div className="rounded-[2rem] bg-white p-6 md:p-8 shadow-sm border border-[#afb3ac]/10">
                                            {isEditing ? (
                                                <textarea
                                                    ...
                                                    placeholder="어떤 태도로 대화하는지 자유롭게 적어보세요."
                                    ...
                                    <section>
                                        ...
                                        <div className="rounded-[2rem] bg-white p-6 md:p-8 shadow-sm border border-[#afb3ac]/10">
                                            {isEditing ? (
                                                renderListEditor("핵심 추억 리스트", editForm.memories, (next) => setEditForm({ ...editForm, memories: next }), "예: 2019년 제주도 바닷가 산책")
                                            ) : (
                                    ...
                                    <section>
                                        ...
                                        <div className="rounded-[2rem] bg-white p-6 md:p-8 shadow-sm border border-[#afb3ac]/10">
                                            {isEditing ? (
                                                renderListEditor("자주 쓰는 문구", editForm.frequentPhrases, (next) => setEditForm({ ...editForm, frequentPhrases: next }), "예: 밥 먹었어?")
                                            ) : (
