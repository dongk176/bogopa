...
export function StartChatButtonDesktop() {
    ...
    if (session) {
        return (
            <Link
                href="/step-1/start"
                className="group relative hidden overflow-hidden rounded-full bg-gradient-to-r from-[#3e5560] to-[#4a626d] px-10 py-4 text-lg font-bold text-[#f0f9ff] transition-all duration-500 hover:-translate-y-1 hover:scale-105 active:scale-[0.98] md:inline-flex cursor-pointer"
            >
    ...
    return (
        <>
            ...
            <LoginModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} nextPath="/step-1/start" />
        </>
    );
}
...
export function StartChatButtonMobile() {
    ...
    return (
        <>
            <button
                onClick={() => setIsModalOpen(true)}
                className="group fixed bottom-[calc(6.5rem+env(safe-area-inset-bottom))] left-4 right-4 z-[60] overflow-hidden rounded-full bg-gradient-to-r from-[#3e5560] to-[#4a626d] px-6 py-4 text-center text-base font-bold tracking-wide text-[#f0f9ff] transition-all duration-500 hover:-translate-y-1 hover:scale-[1.02] active:scale-[0.98] md:hidden cursor-pointer"
            >
